...
class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"
                                ...
                                <svg
class="mr-3 h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg"
...
fill-rule="evenodd"
                            ...
                            <input
type="text" v-model="searchTerm" name="search" id="search"
...
const emit = defineEmits(["selections-updated", "asset-doubleclicked", "navigated"]);
 ...
 */
const dragOver = () => {
    return draggingFile.value = true;
};
 ...
 */
const dragStop = () => {
    return draggingFile.value = false;
};
...
const assetSelected = (asset) => {
    ...
    if (_.contains(assetsSelected.value, asset.id)) {
        return;
    }

    assetsSelected.value.push(asset.id);
